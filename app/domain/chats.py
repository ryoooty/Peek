...
async def _collect_context(
    ...
    *,
    ...
    if threshold and total_tokens > threshold:
        ...
        cache = storage.get_chat_cache_tokens(chat_id)
        _apply_billing(user_id, model, summary.usage_in, summary.usage_out, cache)
        storage.add_chat_cache_tokens(chat_id, -summary.usage_in + summary.usage_out)
        tail = res[1:][-20:]
        res = [
...
def _apply_billing(
    user_id: int, model: str, usage_in: int, usage_out: int, cache_tokens: int = 0
) -> Tuple[int, int]:
...
async def chat_turn(user_id: int, chat_id: int, text: str) -> ChatReply:
    ...
    cache_before = storage.get_chat_cache_tokens(chat_id)
    ...
    storage.add_chat_cache_tokens(chat_id, usage_in + usage_out)
