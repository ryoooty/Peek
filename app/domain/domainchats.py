...
async def summarize_chat(chat_id: int, *, model: str, sentences: int = 4) -> str:
    ...
    r = await provider_chat(model=model, messages=messages, temperature=0.3, max_tokens=220, timeout_s=settings.limits.request_timeout_seconds)
...
async def chat_turn(user_id: int, chat_id: int, text: str) -> ChatReply:
    ...
    r = await provider_chat(model=model, messages=messages, temperature=0.7, max_tokens=toks_limit, timeout_s=settings.limits.request_timeout_seconds)
    ...
    return ChatReply(text=out_text, usage_in=int(r.usage_in or 0), usage_out=int(r.usage_out or 0), billed=billed, deficit=deficit)

async def live_stream(user_id: int, chat_id: int, text: str) -> AsyncGenerator[Dict[str, str], None]:
    """
    Live-режим: отдаём куски по мере готовности.
    Возвращает генератор словарей:
      {"kind":"chunk","text": "..."} — очередной кусок
      {"kind":"final","text":"...","usage_in":X,"usage_out":Y,"billed":Z,"deficit":D}
    """
    user = storage.get_user(user_id) or {}
    ch = storage.get_chat(chat_id) or {}
    resp_size = (ch.get("resp_size") or user.get("default_resp_size") or "auto")
    ...
    async for ev in provider_stream(model=model, messages=messages, temperature=0.7, max_tokens=toks_limit, timeout_s=settings.limits.request_timeout_seconds):
        if ev.get("type") == "delta":
            part = ev.get("text") or ""
            full += part
            # отдаём части, используя простую эвристику границ (передаём как есть — нарезка на стороне handler)
            yield {"kind": "chunk", "text": part}
    ...
    yield {"kind": "final", "text": full, "usage_in": str(usage_in), "usage_out": str(usage_out), "billed": str(billed), "deficit": str(deficit)}
